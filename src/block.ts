...
export class Block {
    ...
    constructor(
        ...
        logger.debug(`Canonical block: ${canonicalize(this.toNetworkObject())}`);
    }
    ...
    async getTxs(peer?: Peer): Promise<Transaction[]> {
        ...
        for (const maybeTx of maybeTransactions) {
            if (!TransactionObject.guard(maybeTx)) {
                throw new Error(`Block reports a transaction with id ${objectManager.id(maybeTx)}, but this is not a transaction.`)
            }
    ...
    async validateTx(peer: Peer, stateBefore: UTXOSet, height: number) {
        ...
        if (coinbase !== undefined) {
            ...
            if (coinbase.height !== height) {
                throw new Error(`Coinbase transaction ${coinbase.txid} of block ${this.blockid} indicates height ${coinbase.height}, `
    ...
    async validate(peer: Peer) {
        ...
        if (blockManager.deferredValidations[this.blockid] !== undefined) {
            ...
            if (!result) {
                throw new Error(`Block validation failure received through propagation.`)
            }
            await this.load()
            return
        }
        ...
        try {
            ...
            if (this.isGenesis()) {
                ...
                if (parentHeight === undefined) {
                    throw new Error(`Parent block ${parentBlock.blockid} of block ${this.blockid} has no known height`)
                }

                if (parentBlock.created >= this.created) {
                    throw new Error(`Parent block ${parentBlock.blockid} created at ${parentBlock.created} has future timestamp of `
